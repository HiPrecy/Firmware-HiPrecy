...
#include "serial.h"
  ...
  bool touch_lcd::test = false;
  volatile bool touch_lcd::ftRecvData = false;
  uint8_t touch_lcd::ftDataLen = 0;
  uint16_t touch_lcd::ftAddr = 0;
  uint8_t touch_lcd::ftData[128];


  void touch_lcd::ftBegin() {
  ...
  bool touch_lcd::waitData(uint8_t*dat) {
    uint16_t t=10000;
    while (t && !ftRecvData) {
      t--;
      DELAY_US(10);
  ...
  void touch_lcd::ftPutPGM(const char* str){
    while (char ch = pgm_read_byte(str++)) cdt[n++] = ch;
    for (; n < 8; n++)cdt[n] = ' ';
  }

  void touch_lcd::ftSendCmdFromString(const char*tar) {
    char c, ch, tt = 0;
    for (; (*tar >= 'A'&&*tar <= 'F') \
            || (*tar >= 'a'&&*tar <= 'f') \
            || (*tar >= '0'&&*tar <= '9') \
  ...
  const char* touch_lcd::ftCmdString(uint8_t& stringLen) {
    if (ftDataLen<4) {
      stringLen = 0;
      return nullptr;
    }
